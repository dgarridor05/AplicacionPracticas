...
def normalize_text(text):
    """Elimina tildes, espacios extra y lo pasa a minúsculas para comparaciones precisas."""
    if not text:
        return ""
...
def get_ajax_response(request, success, message, prefix, extra_data=None):
    """Genera una respuesta JSON estandarizada para todos los minijuegos."""
    data = {
        'success': success,
        'message': message,
...
def face_guess_game(request):
    """Juego: Adivinar el nombre viendo la foto."""
    groups = ClassGroup.objects.filter(teacher=request.user)
    students = UserProfile.objects.filter(
        role='student', student_groups__in=groups, profile_picture__isnull=False
    ...
    if request.method == 'POST':
        ...
        if is_correct:
            ...
            msg = f"¡Correcto! Es {student.full_name or student.username}"
        else:
            msg = f"Incorrecto. Era: {student.full_name or student.username}"
...
def name_to_face_game(request):
    """Juego: Elegir la foto correcta dado un nombre."""
    groups = ClassGroup.objects.filter(teacher=request.user)
    students = UserProfile.objects.filter(
        role='student', student_groups__in=groups, profile_picture__isnull=False
    ).exclude(profile_picture='').distinct()
    ...
    if request.method == 'POST':
        ...
        is_correct = str(selected_id) == str(target_id)
        if is_correct:
            request.session['name_to_face_correct'] += 1
            msg = "¡Correcto! Has reconocido al alumno."
        else:
            target_s = UserProfile.objects.get(id=target_id)
            msg = f"Incorrecto. Era {target_s.full_name or target_s.username}."
...
def hangman_game(request):
    """Juego del Ahorcado con soporte completo AJAX."""
    groups = ClassGroup.objects.filter(teacher=request.user)
    ...
    if request.method == 'POST' and request.POST.get('action') == 'new_game' or 'hangman_target_id' not in request.session:
        ...
        if request.POST.get('action') == 'new_game':
    ...
    if request.method == 'POST' and request.POST.get('action') == 'guess_letter':
        ...
        if not request.session['hangman_game_over'] and letter not in request.session['hangman_guessed_letters']:
            ...
            lost = request.session['hangman_incorrect_count'] >= max_incorrect
...
def student_complete_profile_game(request):
    """Juego: Identificar la ficha técnica completa de un alumno."""
    groups = ClassGroup.objects.filter(teacher=request.user)
    ...
    if request.method == 'POST':
        ...
        msg = "¡Perfil identificado!" if is_correct else "Ese no era el perfil correcto."
        if request.headers.get('x-requested-with') == 'XMLHttpRequest':
            return get_ajax_response(request, is_correct, msg, 'complete_profile')
    ...
    for s in raw_options:
        ...
        options_data.append({
            ...
            'nickname': s.nickname or "Sin apodo",
            'age': calculate_age(s.date_of_birth),
            'favorite_artist': s.favorite_artist or "No especificado",
            'favorite_movie': s.favorite_movie or "No especificada",
            'vark_result': vark.dominant_category if vark else "Pendiente",
            'chapman_result': chapman.dominant_category if chapman else "Pendiente",
            'motivation': s.motivation or "Sin descripción."
        })
